...
in
{
  ...
  # Used only from the laptop by shelly/deploy.sh.
  "hue-shelly-key.age".publicKeys = users;

  # Used only from the laptop by shelly/*.sh; HA keeps its own copy in .storage.
  "shelly-auth.age".publicKeys = users;
